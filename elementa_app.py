...
def calibration_fig_to_png_matplotlib(cal_result: dict, concs, signals,
    ...
    """
    Genera la curva de calibración como PNG con matplotlib.
    Usa plt.switch_backend('agg') para compatibilidad total con Streamlit.
    No requiere kaleido.
    """
    try:
        ...
        RED_C = "#f87171"; ORANGE = "#fb923c"
        SUBTEXT = "#94a3b8"; TEXT = "#e2e8f0"

        fig, ax = plt.subplots(figsize=(7.8, 3.8))
        fig.patch.set_facecolor(BG)
        ax.set_facecolor(BG)

        ax.scatter(concs, signals, color=GREEN, s=60, zorder=5,
                   edgecolors=CARD, linewidths=1.2, label="Estándares")
        ...
        ax.plot(xl, cal_result["slope"]*xl + cal_result["intercept"],
                color=BLUE, linewidth=2.2, label="Regresión lineal")

        y_all = np.concatenate([signals, cal_result["slope"]*xl + cal_result["intercept"]])
        ...
        if lod_ok:
            ax.axvline(lod, color=RED_C, linestyle=":", linewidth=1.3)
            ax.text(lod, ylbl, f"  LOD={lod:.3f}", color=RED_C, fontsize=7, va="bottom")
        if loq_ok:
            ax.axvline(loq, color=ORANGE, linestyle=":", linewidth=1.3)
            ax.text(loq, ylbl, f"  LOQ={loq:.3f}", color=ORANGE, fontsize=7, va="bottom")

        m, b, r2 = cal_result["slope"], cal_result["intercept"], cal_result["r2"]
        sgn = "+" if b >= 0 else "-"
        eq  = f"y = {m:.4f}x {sgn} {abs(b):.4f}   |   R² = {r2:.5f}"
        ax.text(0.03, 0.97, eq, transform=ax.transAxes, fontsize=8.5,
        ...
        leg = ax.legend(facecolor=CARD, edgecolor="#334155", fontsize=8)
...
def generate_pdf_report(analyte: str, method: str,
    ...
    RED_D   = HexColor("#450a0a")
    RED_L   = HexColor("#fca5a5")
    NOTE_BG = HexColor("#e8edf3")   # fondo claro para notas explicativas
    NOTE_TX = HexColor("#0a0a0a")   # texto negro para notas
    ...
    note_inner = ParagraphStyle("NI", parent=styles["BodyText"],
        textColor=NOTE_TX, fontSize=8, leading=12.5, fontName="Helvetica-Bold")

    def note_box(text):
        """Caja de nota: fondo claro, texto negro en negritas."""
        t = Table([[Paragraph(text, note_inner)]], colWidths=[7.3*inch])
    ...
    if cal_result:
        ...
        for param, value, explanation in cal_items:
            block = [
                ...
                [
                    Paragraph(explanation, ParagraphStyle("EX", textColor=NOTE_TX, fontSize=7.8, leading=12, fontName="Helvetica-Bold")),
                    "",
                ],
            ]
        ...
        if cal_result.get("lod_proxy"):
            story.append(note_box(
                "Nota: LOD y LOQ calculados usando el error estándar residual como proxy de sigma_blanco. "
                "Para mayor rigor, incluya al menos 10 réplicas del blanco y use su desviación estándar."))
        story.append(Spacer(1, 8))
        ...
        if fig_cal is not None:
            ...
            png_bytes = None
            # 1. Matplotlib (siempre disponible, sin kaleido)
            if cal_concs is not None and cal_signals is not None:
            ...
            if png_bytes:
                ...
                story.append(rl_cal)
                story.append(note_box(
                    "La línea azul es la regresión lineal ajustada. "
                    "Los marcadores verdes son los estándares. "
                    "Las líneas verticales punteadas indican el LOD y el LOQ."))
            else:
                story.append(note_box(
                    "Calibre el método (Paso 4) antes de exportar para incluir la gráfica. "
                    "Si el problema persiste, verifique que matplotlib esté instalado."))
            story.append(Spacer(1, 8))
    ...
    if triplicate_stats_df is not None and not triplicate_stats_df.empty:
        ...
        story.append(note_box(STAT_EXPLANATIONS["CV"]))
        story.append(Spacer(1, 4))
    ...
    if df_results is not None and not df_results.empty:
        ...
        story.append(note_box(
            "Conc_calc: calculada de la curva (x = (y-b)/m). "
            "Conc_corregida: multiplicada por el factor de dilución. "
            "A_dig = log10(I_blanco / I_muestra) en el canal seleccionado."))
        story.append(Spacer(1, 4))
    ...
    story.append(note_box(
        "<b>Nota científica:</b> La precisión depende de la iluminación, el sensor, los reactivos "
        "y la preparación de estándares. Para cumplimiento normativo confirmar mediante métodos "
        "oficiales en laboratorios acreditados. Consultar siempre la versión vigente del DOF."))
    story.append(Spacer(1, 8))
...
if pagina == "Analisis":
    ...
    # Sincronizar assignments_dict para uso posterior en la página
    assignments_dict = {}
    if st.session_state["assignment_df"] is not None:
        for _, row in st.session_state["assignment_df"].iterrows():
            assignments_dict[row["ROI"]] = row.get("Tipo", "Sin asignar")
...
**Triplicados:** en diseño de microplaca, los pocillos de la misma columna (p. ej. A1, B1, C1) se tratan automáticamente como réplicas de la misma condición.
    ...
    # Persistencia robusta: solo recrear si la estructura de ROIs cambió
    roi_fp = hashlib.md5("|".join(r["label"] for r in rois).encode()).hexdigest()[:12]
    if (st.session_state["assignment_df"] is None or
            st.session_state.get("_asgn_fp") != roi_fp):
        old_map = {}
        if st.session_state["assignment_df"] is not None:
            for _, r in st.session_state["assignment_df"].iterrows():
                old_map[r["ROI"]] = r.to_dict()
        init_rows = []
        for roi in rois:
            if roi["label"] in old_map:
                init_rows.append(old_map[roi["label"]])
            else:
                init_rows.append({
        ...
        st.session_state["_asgn_fp"] = roi_fp
    ...
    edited_df = st.data_editor(
        ...
        key=f"assignment_editor_{roi_fp}",
    )
    ...
    if st.button("Extraer RGB y calibrar"):
        with st.spinner("Procesando canales de color y ajuste de calibracion..."):
            ...
            if best_ch in all_chs and len(std_df) >= 2:
                ...
                # Datos para el gráfico matplotlib del PDF
                st.session_state["_cal_concs"]   = concs
                st.session_state["_cal_signals"] = signals
                st.session_state["_cal_unit"]    = unit
                st.session_state["_cal_analyte"] = analyte
                st.session_state["_cal_ch"]      = best_ch
    ...
    if st.button("Generar reporte PDF"):
        ...
        try:
            pdf_bytes = generate_pdf_report(
                ...
                cal_concs           = st.session_state.get("_cal_concs"),
                cal_signals         = st.session_state.get("_cal_signals"),
                cal_channel         = st.session_state.get("_cal_ch", ""),
                cal_analyte         = st.session_state.get("_cal_analyte", analyte_rep),
                cal_unit            = st.session_state.get("_cal_unit", "mg/L"),
            )
